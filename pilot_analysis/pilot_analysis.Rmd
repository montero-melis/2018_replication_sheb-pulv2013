...
At the same time, we have seen that a verb that is not well understood also
...
```{r, message = FALSE, warning = FALSE}
## arm verbs
bias_arm <- bias[bias$type == "arm", c("verb", "type", "bias_absol")]
bias_arm <- bias_arm[order(-bias_arm$bias_absol),]
bias_arm <- left_join(bias_arm, und_free_acc)
bias_arm$rank <- 1:nrow(bias_arm)
# reorganize and rename columns
bias_arm <- bias_arm %>%
  ungroup() %>%
  select(rank, arm_verb = verb, arm_bias = bias_absol, arm_acc = accuracy)
## leg verbs
bias_leg <- bias[bias$type == "leg", c("verb", "type", "bias_absol")]
bias_leg <- bias_leg[order(-bias_leg$bias_absol),]
bias_leg <- left_join(bias_leg, und_free_acc)
bias_leg$rank <- 1:nrow(bias_leg)
# reorganize and rename columns
bias_leg <- bias_leg %>%
  ungroup() %>%
  select(rank, leg_verb = verb, leg_bias = bias_absol, leg_acc = accuracy)
# join
verb_choice <- left_join(bias_arm, bias_leg)
```
