...
We need to match arm and leg verbs with respect to the variables that Shebani
...
```{r}
select_sw <- bias_sw %>% filter(bias_absol >= cutoff_swe)
select_sw$verb <- as.character(select_sw$verb)  # prevents unexpected problems
```
...
We select all leg verbs and 64 arm verbs, for a total of 120 verbs.

```{r}
# 64 Swedish arm verbs with strongest arm bias:
sw_top64_arm <- as.character(pull(select_sw[order(- select_sw$bias), ], verb))[1:64]
select_sw <- select_sw %>%
...
Save this list to disk for furhter norming:
