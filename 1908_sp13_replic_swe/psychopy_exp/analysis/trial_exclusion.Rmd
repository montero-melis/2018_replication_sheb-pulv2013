...
Applies the second exclusion criterion indicated in our Registered Report
(https://osf.io/26fbh/, p.22):
...
Extract tapping information from a log file:

```{r}
log_ex <- extract_tap_info("../data/1_sp13_replication_swe_2020_Oct_30_1334.log")
```
...
2 and 3 from the arm block would have to be excluded.

The following function computes this (we show only the 6 first trials of the
arm block, which was the 2nd block):
...
ppt_data <- read_csv("../participant_data_210609.csv") %>%
  select(1:2)
...
```{r}
write_csv(excluded_trials, "excluded_trials_bc_of_paradiddle.csv")
```
