...
# 1) "und" for understanding, "mc"" for multiple choice
und_mc <- read.csv("data_verb-understanding_multiple-choice.csv",
                    fileEncoding = "UTF-8")
# 2) "und" for understanding, "fr"" for free
...
The analyses reported below are quite detailed. All of the data from the pilot
study is explored mainly with visualizations, but a couple of mixed model 
analyses are reported as an appendix (they just corroborate the figures).

Here I summarize the main results and what lessons I think we should extract.
For details, please look up the corresponding sections below.


## Conclusions from memory task

*Quadruple vs by-verb scoring*

I considered two types of scoring procedures. 
Either the scoring was done at the **level of the quadruples** that made up
a trial (counting a trial as correct *only* when all of the verbs were 
remembered in the correct order, and as wrong otherwise),
or the scoring was done at the **level of individual verbs**, in which case
we coded if each verb was remembered correctly (=1) or not (=0).

Results:

1) If we adopt quadruple-level scoring, then error rates are substantally higher
for our L2 speakers than for the native speakers reported in the original
Shebani and Pulvermüller (2013 [S&B]). Their native speakers had error rates of
around 30% (in the control condition, which is the right comparison), whereas
L2 speakers lay around 50%.
1) If we adopt a verb-level scoring, then error rates drop to about 20% instead
(we have no way of comparing this with the native speakers from S&B).
1) The two ways of scoring correlated very highly when we computed mean error
rates by participant. This suggests that using one or the other will not order
participants differently. Yet, we may assume that the by-verb scoring is a
more sensitive measure (it contains more information).
1) Note that by far the most common errors were omission and replacement; both
of these error types are also captured when scoring at the verb level (whereas
shuffling the order of the verbs would not -- but that was uncommon).

*Verb presentation rate had almost no effect*

Our manipulation of verb presentation rates (100, 200, 300 and 400 ms) had
almost no effect on accuracy. There might be a very slight advantage of
having 400 ms to read the word compared to 100 ms, but I don't think it's
worth changing this aspect of the original study for a very slight increase
in accuracy.
Instead, I suggest we adopt the by-verb type of rating, which also decreases
error-rates but should not affect our sensitivity to find an embodiment
effect.

*Verb type (arm-/leg-related) had no effect on error rate*

Whether verbs were arm- or leg-related did not seem to impact performance
on the memory task.


## Conclusions from verb norming
...
1) Our arm verbs elicited a stronger bias than our leg verbs.
2) We should keep in mind that even relatively high average bias ratings
can hide considerable variability in the ratings given by each individual.


## Conclusions from verb comprehension task

We had two versions of the comprehension task, a **multiple choice** task
and a **free translation** task. Each participant did either one or the other
version.
Conclusions:

1) The multiple choice task has methodological problems, making it difficult
to draw inferences from this data. (Unfortunately most participants did this
version).
2) We therefore base our conclusions on the free translation task (N = 6).
3) Most of the verbs were known
(mean translation accuracy `r round(mean(und_free$score),2)`),
but leg verbs were less known to participants than arm verbs
(mean accuracy of
`r round(mean(und_free[und_free$type == "leg", "score"]),2)` and
`r round(mean(und_free[und_free$type == "arm", "score"]),2)`,
respectively).


## Overall suggestions based on pilot study

1) Keep the presentation rate as in original: each word displayed for 100 ms
and 400 ms blank between words.
2) Adopt a scoring at the verb-level. This allows us to take item (i.e., verb)
variability into account. We can then also use some properties of specific
verbs as covariates (e.g., frequency, cognate status). Finally, it will also
allow for a better randomization procedure of items.
3) We need more leg verbs that elicit a strong bias and are understood by
L2 speakers. We have 54 arm verbs that have a clear bias and were understood at
least by 50% of the participants; but there are only 37 such verbs in the leg
category.
...
We will call each set of four verbs that made up a trial a **quadruple** 
(this can also be understood as an item, but because we will analyze the data
at the level of quadruple *and* at the level of individual verbs, we use 
the term quadruple to avoid ambiguity).

Quadruples consist of either *arm-related verbs* (e.g., grab, dunk, lift, fold) or *leg-related verbs* (e.g., wander, step, stride, limp).

```{r include=FALSE}
table(unique(meml[, c("type", "verb")])$type)
```

Each participant carried out 4 blocks in which they had to memorize 28 
quadruples altogether, 14 quadruples of arm-related, and 14 quadruples of 
leg-related verbs.
...
The time between the offset of a verb and the onset of the next verb was held
constant at 400 ms.

We will analyze the data in three ways:

1. Looking at by-subject averages when we consider whole quadruples, i.e. we
count a response as correct only if all four verbs in the quadruple were 
remembered in the correct order;
2. Looking at by-subject averages computed at the levell of individual verbs,
that is, by looking whether each individual verb in a quadruple was remembered
or not.
3. Considering by-verb variability, i.e. as a by-item analysis.


## By-subject analysis at the level of *quadruples*

### Nature of the data (wide format)

Here is an example of the data set when analyzed by quadruples:
...
- It corresponds to `participant`'s 900 first quadruple (`trial` = 1) in the
first block (`block` = 1).
- In that block each word was shown for 200 ms (`word_duration` = 200).
- It was a quadruple of arm-related verbs (`type` = arm) and the four words shown
were *grab*, *dunk*, *lift* and *file* (`word1`--`word4`).
- The participant did not make any error (`error` is blank) and thus achieved
a `score` of 1.

The `score` column rates quadruples as either correct (=1) if all verbs are
remembered in the right order, or as wrong (=0) if some error was made by the
participant.
The `error`  column contains abbreviations for the types of error we coded for
(following Shebani & Pulvermüller, 2013 [S&P]):
...
presentation rate.
Observations were always removed at the level of quadruples.
For example, if one word was unintelligible, then the whole quadruple (that is,
all four verbs in that trial) was deleted.
This results in the following number of quadruples per participant and word
presentation rate:
...
ggplot(ppt_mean_ty, aes(x = word_duration, y = error_rate, colour = type)) +
  stat_summary(fun.data = "mean_cl_boot", size = 1.5, position = position_dodge(width = 30)) +
  geom_jitter(width = 20, height = 0, alpha = .4) +
  ylab("Error rate") + xlab("word duration (in ms)") +
  ggtitle("Average by-subject error rates as a function of presentation rate and type\n(quadruple scoring)") +
  ylim(ylims)
```
...
ggplot(ppt_mean_ty, aes(x = block, y = error_rate, colour = type)) +
  stat_summary(fun.data = "mean_cl_boot", size = 1.5, position = position_dodge(width = .3)) +
  geom_jitter(width = .2, height = 0, alpha = .4) +
  ylab("Error rate") + 
  ggtitle("Average by-subject error rates as a function of block and type\n(quadruple scoring)") +
  ylim(ylims)
```
...
of all the quadruples that were not remembered correctly.


### Participant variability

The plots in the previous section suggest substantial participant variability.
Let us look at this by plotting each participant's mean error rate (averaged
across blocks).
...
ggplot(ppt_mean_simp, aes(x = participant, y = error_rate)) +
  ...
  ylab("Error rate") +
  ggtitle("Participant variability:\nAccuracy scored at the quadruple level") +
  ylim(ylims)
```
...
ggplot(ppt_mean_simp_type, aes(x = arm, y = leg)) +
  geom_point() +
  xlab("Error rate for arm quadruples") + ylab("Error rate for leg quadruples") + 
  ggtitle("Correlation of mean error rates between verb types\n(quadruple scoring)") +
  geom_smooth(method = "lm")
```

The correlation is indeed high 
(*r* = `r round(with(ppt_mean_simp_type, cor(arm, leg)), 2)`),
suggesting that participants ordered in similar ways with respect to 
error rates for the two verb types; that is, it was not the case that
...
```{r, eval = TRUE, fig.height = ggpairs_he}
ggpairs(ppt_mean_simp_durat[, 2:5], lower = list(continuous = "smooth")) +
  ggtitle("Correlation of mean by-subject error rates across presentation rates\n(quadruple scoring)")
```
...
ggpairs(ppt_mean_simp_block[, 2:5], lower = list(continuous = "smooth")) +
  ggtitle("Correlation of mean by-subject error rates across blocks\n(quadruple scoring)")
```
...
Instead of scoring whole quadruples as correct or incorrect, we now consider
individual verbs.
If a verb within a quadruple was remembered correctly, it is scored as correct
(=1), if it is not remembered, it gets a score of zero.
This means, for instance, that:

- A quadruple for which 3 of the 4 verbs were remembered yields a 25%
error rate under this coding, whereas it led to a 100% error rate under the
more severe coding by quadruple.
- A quadruple for which all verbs were remembered, but in a shuffled order, would
be scored as completely accurate under the verb-level coding, but as completely
wrong under the quadruple coding.
...
To look at the data by verb rather than by quadruple, we need the data in long format:
...
first quadruple of the first block, which were all remembered correctly by 
participant 900.
...
We begin with a plot equivalent to the one we did for whole quadruples, but now
we show the error rates when we consider whether individual verbs were remembered:
...
### Effect of verb position within a quadruple

Was the position in which a verb occured in the quadruple (word 1--4) predictive
of how well it was remembered?
This seems likely, as we know that in lists the first and last items are better
...
ggplot(ppt_mean_l_wordInTrial, aes(x = wordInTrial, y = error_rate)) +
  ...
  ggtitle("Error rate as a function of verb position in quadruple") +
  ylim(ylims2)
```
...
ggplot(ppt_mean_l_wordInTrial_durat, aes(x = wordInTrial, y = error_rate)) +
  ...
  ylab("Error rate") + xlab("Verb position")  +
  ggtitle("Error rate as a function of verb position in quadruple and presentation rate") +
  ylim(0, .6)
```
...
ggplot(ppt_mean_l_wordInTrial_block, aes(x = wordInTrial, y = error_rate)) +
  ...
  facet_grid(. ~ block) +
  ylab("Error rate") + xlab("Verb position") +
  ggtitle("Error rate as a function of verb position in quadruple and block") +
  ylim(0, .6)
```
...
The take-away message is that it is important to randomize the verbs within
a quadruple. 
(Or alternatively: completely randomize the order of verbs while still forming
quadruples by grouping same-type verbs, as we did in this pilot.)
...
ggplot(ppt_mean_l_simp, aes(x = participant, y = error_rate)) +
  geom_bar(stat = "identity") +
  ylab("Error rate") +
  ggtitle("Participant variability:\nAccuracy scored at the verb level") +
  ylim(ylims)
```
...
For comparison, we plot again variability when computed by quadruples
...
We can check whether the two ways of measuring error rates correlate well within 
participants:
...
ggplot(error_rates, aes(x = error_rate_verb, y = error_rate)) +
  geom_point() +
  xlab("Error rate for individual verbs") + ylab("Error rate for quadruples") + 
  ggtitle("Correlation between error rates computed by quadruples and by verbs") +
  geom_smooth(method = "lm")
```
...
To do that, we look at whether individual verbs (not quadruples) were remembered 
correctly.
...
The average error rate of a verb is the mean number of participants who 
...
ggplot(item_mean_simp, aes(x = verb, y = error_rate, fill = type)) +
  geom_bar(stat = "identity") +
  ylab("Error rate") +
  ggtitle("Verb variability") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 8)) +
  ylim(ylims2)
```

Variability by verbs is similar in magnitude to variability between participants
when this latter is computed as error rates at the verb level
...
ggplot(item_mean_simp, aes(x = type, y = error_rate, colour = type)) +
  geom_boxplot() +
  ylab("Error rate") +
  ggtitle("Average by-verb error rates for the two types")
```
...
### By-verb correlation of error rates between different word presentation rates

We use a scatterplot matrix to check whether the by-verb correlations of error
...
Not sure how surprising this is, but the correlations of by-verb means are
...
ggplot(bias, aes(x = type, y = bias_absol, colour = type)) + 
  ...
  ggtitle("Size of congruent bias of verbs")
```
...
Save the dataframe containing verb biases (`bias`) to disk:

```{r}
write.csv(bias, "verb_bias_mean.csv", row.names = FALSE, fileEncoding = "UTF-8")
```
...
ggplot(bias_spread, aes(x = verb, y = bias, colour = type)) +
  ...
  ylab("Verb bias") + 
  coord_flip() +
  ggtitle("Variability around mean arm/leg bias of verbs")
```
...
ggplot(und_mc_acc, aes(x = verb, y = accuracy, colour = type)) +
  ...
  ylim(0,1) +
  coord_flip() +
  ggtitle("Translation accuracy of verbs in multiple choice version")
```
...
perhaps some verbs *were* known, but the alternative given as the correct one
...
diff_verbs <- diff_verbs[order(diff_verbs$accuracy), ]
kable(diff_verbs)
# save as vector
diff_verbs_v <- as.character(diff_verbs$verb)
```


This is surprising, especially for verbs like *roll* or *scratch* since one
might expect Swedish L2 speakers of English to know the meaning of these verbs.

So let us look at the target and distractor items for these verbs:

```{r, message = FALSE, warning = FALSE}
kable(inner_join(diff_verbs, und_mc %>% select(verb:distrTwo) %>% unique))
```
...
```{r}
kable(head(und_free[und_free$score_unsimplified == -99, c(3:5, 11, 7:8)], 3))
```
...
We will consider translations scored as $-99$ as incorrect, which is the more
conservative choice. (NB: After checking, there are not drastic changes if we
instead count the unclear cases as correct; it leads to *very* minor changes.)

```{r}
# This is done in the header of this document, after loading the data
```
...
ggplot(und_free_acc, aes(x = verb, y = accuracy, colour = type)) +
  ...
  ylim(0,1) +
  coord_flip() +
  ggtitle("Translation accuracy of verbs in free translation version")
```
...
correct but unrelated to the intended meaning (e.g., 'byta').
...
In both plots, we see that the correlation is not very strong. Since
accuracy is not evenly spread (most verbs have an accuracy of 1 or close),
the correlation is pretty much driven by the few more extreme observations.
...
We now combine all the information about the individual verbs:
...
(To combine this info, here is a very useful 
[cheat sheet for dplyr join functions](http://stat545.com/bit001_dplyr-cheatsheet.html)):
...
in any of the two translation tasks ([3,1] and [4,1]).
The numerical values of these correlations are shown in panels [1,2]--[1,4].
...
should also lead to a low absolute bias, and vice versa.
...
## Memory task: accuracy rated at quadruple level
