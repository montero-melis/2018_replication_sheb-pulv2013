...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'C:\\Users\\gumo8029\\Documents\\gitrepos\\2018_replication_sheb-pulv2013\\exp-scripts_psychopy\\sheb_replic.psyexp',
...
# Initialize components for Routine "init_exp_vars"
init_exp_varsClock = core.Clock()
## Set global (experiment-level) variables

pptID = expInfo['participant']  # participant ID
...
text_4 = visual.TextStim(win=win, name='text_4',
    text='You will start a training phase now.\n\nPress SPACE BAR to continue',
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1,
    depth=0.0);
...
# ------Prepare to start Routine "init_exp_vars"-------
t = 0
init_exp_varsClock.reset()  # clock
frameN = -1
continueRoutine = True
# update component parameters for each repeat

# keep track of which components have finished
init_exp_varsComponents = []
for thisComponent in init_exp_varsComponents:
    if hasattr(thisComponent, 'status'):
        thisComponent.status = NOT_STARTED

# -------Start Routine "init_exp_vars"-------
while continueRoutine:
    # get current time
    t = init_exp_varsClock.getTime()
...
# -------Ending Routine "init_exp_vars"-------
for thisComponent in init_exp_varsComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)

# the Routine "init_exp_vars" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()
...
# update component parameters for each repeat
press_space = event.BuilderKeyResponse()
# keep track of which components have finished
instr_welcomeComponents = [welcome, press_space]
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    frameRemains = 0.0 + 10- win.monitorFramePeriod * 0.75  # most of one frame period left
    if press_space.status == STARTED and t >= frameRemains:
        press_space.status = STOPPED
    if press_space.status == STARTED:
        ...
        if len(theseKeys) > 0:  # at least one key was pressed
            press_space.keys = theseKeys[-1]  # just the last key pressed
            press_space.rt = press_space.clock.getTime()
...
block = data.TrialHandler(nReps=1, method='random', 
    ...
    trialList=data.importConditions(u'conditions_block.csv'),
...
for thisBlock in block:
    ...
    print("Printing stuff at the beginning of block")
    print("block.thisN is: " + `block.thisN`)
    ...
    # set up handler to look after randomisation of conditions etc
    training_block = data.TrialHandler(nReps=10, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
    ...
    for thisTraining_block in training_block:
        ...
        word_presentation_training = data.TrialHandler(nReps=1, method='random', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions(u'conditions_training_test.csv'),
        ...
        while continueRoutine:
            ...
            if key_resp_8.status == STARTED:
                theseKeys = event.getKeys(keyList=['t', 's'])
        ...
        if key_resp_8.keys == "s": break  # allow to repeat training until ready (max 10, see loop)
        # the Routine "repeat_training" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
        
    # completed 10 repeats of 'training_block'
    ...
    word_presentation = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(u'conditions_exp-trials_test.csv'),
