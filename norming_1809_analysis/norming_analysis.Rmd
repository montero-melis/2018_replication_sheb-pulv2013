...
`r sum(bias_en$bias_absol < 0)` verbs had opposite bias to what was expected:
...
`r sum(bias_sw$bias_absol < 0)` verbs had opposite bias to what was expected:
...
Number of observations per verb in L2 norming studies (June and September)
------------------------------------------------

We had verbs rated for their arm- vs leg-relatedness on two different
...
Here is a table that shows the number of total observations for each verb,
where one observation consists of two ratings provided by a single participant
(one for arm- the other for leg-relatedness):

```{r}
unique(bias_en_l[, c("participant", "verb", "category")]) %>%
  group_by(verb, category) %>%
  summarise(nbObs = n()) %>%
  arrange(nbObs, category, verb) %>%
  kable
```
