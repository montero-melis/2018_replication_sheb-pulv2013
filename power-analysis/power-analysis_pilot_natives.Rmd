...
Below, we explore two approaches:
...
To get an intuition of the data we are simulating, let's facotr it all into a
...
```{r}
kable(simulate_data()() %>% tail)  # nb_sim defaults to 1
```
...
partitioned in that case: the random by-subject variability for the interaction
...
then be run on the dataframe contained the fitted models to extract summaries
of interest.
...
fit_many <- function(parameterList = NULL, nbSims = NULL, 
  ...
  # By default (if loadOnly = T) it will just load the data
  if (loadOnly) {
  ...
  for (sigma_name in names(parameterList$my_sigma)) {  # only way I found to keep track of name
    sigma <- parameterList$my_sigma[[sigma_name]]  # a bit clumsy
    for (N in parameterList$my_n) {
      data_simulator <- simulate_data(my_n = N, my_sigma = sigma)
...
#                 my_n = c(15, 30)))
# 
(params <- list(my_sigma = list("correl_original" = orig_data_summaries$sigma),
                my_n = c(15, 30)))
```
...
my_simulations <- fit_many(params, nbSims = 209, loadOnly = FALSE)
head(my_simulations)
nrow(my_simulations)
...
load_or_extract_summaries <- function(extract_anew = FALSE) {
  if (! extract_anew) {  # Default will be to just load the stored object
    load("my_fm_summaries.RData", envir = .GlobalEnv)
...
load_or_extract_summaries(extract_anew = FALSE)
...
```{r}
betas <- with(sp_means, c(interce = mean(M),
